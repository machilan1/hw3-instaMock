...
export class MessengerService {
  ...
  getSessionIDByRecieverID(recieverID: string) {
    return this.mocksessions.filter(
      (session) =>
        session.participant.includes(this.clientService.currentClientID) &&
        session.participant.includes(recieverID)
    )[0].sessionID;
  }

  generateMessageObject(senderID: string, content: string) {
    const temp = {
      sender: senderID,
      content: content,
      timeStamp: '99998877',
  ...
  appendNewMessage(senderID: string,recieverID:string, content: string){

    const comment =this.generateMessageObject(senderID, content)
    const sessionID = this.getSessionIDByRecieverID(recieverID)

    this.mocksessions[this.mocksessions.findIndex(x=>x.sessionID===sessionID)].messages.push(comment)
    this.sessions$.next(this.mocksessions)

    console.log(this.mocksessions[this.mocksessions.findIndex(x=>x.sessionID===sessionID)].messages)
